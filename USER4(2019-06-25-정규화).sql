...
-- 시작번호를 100으로 한 이유는
-- 현재 excel의 데이터를 입력해서 임포트를 하려고한다.
-- excel에 입력되어있는 데이터가 일련번호를 21까지 차지하고있기때문에
-- 이후 INSERT를 수행할때 새로운 번호를 100부터 시작하기 위함이다.
...
-- 이 테이블의 조용필 학생의 주소가 null인데 주소가 확인이 되어
-- 주소를 입력하려고한다.
UPDATE tbl_test
SET st_addr = '서울특별시'
WHERE st_name = '조용필';
...
-- 실제 변경하지 말아야할 데이터까지 변경되는 사태가 발생한다.
-- 그래서 다수의 레코드를 변경하는 명령은 매우 신중하게 사용해야한다.
...
-- 과목테이블로 생성을 하고
-- 성적을 조회할때 과목 테이블과 JOIN하여 볼수 있도록 설정할 예정
...
--      일치하는 값이 있으면
--      과목코드를 return한다
-- C. UPDATE 명령은 return 받은 과목코드를 tbl_score sc_sb_no 칼럼에
--      UPDATE를 실행한다.
UPDATE tbl_score SC
SET SC.sc_sb_no =
    (
